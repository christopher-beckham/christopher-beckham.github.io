...
- (4) In the absence of a reward oracle which can be judiciously evaluated, we need to turn to validation metrics, which are assumed to be cheap to compute. ~How can we use such metrics to help us build better generative models?~ Given a list of metrics a-priori, how do we find which ones are the best?
...
To measure how good a validation metric is, we conduct a large-scale study where we train /many/ model configurations, log the value of that metric, and see how all of those values correlate to their corresponding test reward (Eqn. (1)). Therefore, for each validation metric one can imagine many points where the x-axis denotes its validation metric value and the y axis its true reward computed via Eqn. (1) and with respect to the "extrapolated" model $\ptnew(x,y)$.


Concretely, this was done with /denoising diffusion probabilistic models/ (DDPMs) [cite:@ho2020denoising], due to their flexibility and strong performance in generative modeling. Using a fixed DDPM architecture backbone, I varied several hyperparameters such as network width, reward dropout probability[fn:ddpm], and reward guidance strength. Each unique hyperparameter setting defines a /configuration/.

# (So roughly a total of $N_{\text{hyperparams}} \times N_{\text{datasets}} \times N_{\text{metrics}}$ models.) 
...
<figcaption><b>Figure 5.</b> Each dataset is a subfigure, and each column is a validation metric. Each metric is plotted against <i>M_reward</i> (shown here as <i>M_test_reward</i>), and points are colour-coded according to a diffusion-model specific generation parameter called label guidance (which can be safely ignored for this figure). The Pearson correlation ρ is shown above each plot. Since each validation metric in this figure is designed to be minimised (negative values are added to metrics which typically are maximised), we are interested in metrics which are most negatively correlated, i.e. ones which give rise to the most negative values of ρ.</figcaption>
<br />
</div>
#+END_EXPORT
